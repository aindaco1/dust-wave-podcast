...
| Full private listen and promote/reject decision for the ready enhanced derivative | Normalized delivery-audio render | Awaiting human review. The content-free staging gate intentionally orders this decision before delivery rendering so a master replacement cannot immediately stale the MP3 and player peaks |
...
| Turnstile key pairs | Staging admin login and first production deploy | Dedicated staging pair is operational. Dedicated production widget is created for `dustwave.xyz` and `www.dustwave.xyz`; install its retrievable secret only when the production Worker is first created |
