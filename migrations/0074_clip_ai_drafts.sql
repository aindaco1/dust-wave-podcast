...
ALTER TABLE editorial_ai_drafts RENAME TO editorial_ai_drafts_0073;

-- Keep every AI proposal private and review-only while allowing clip
-- proposals to pin the same exact human-approved word-alignment evidence as
-- chapter proposals.
CREATE TABLE editorial_ai_drafts (
  ...
  kind TEXT NOT NULL CHECK (kind IN ('show_notes', 'chapters', 'clips')),
  ...
  CHECK (
    ...
    OR source_alignment_revision_id IS NOT NULL
  ),
  CHECK (
...
SELECT
  ...
  lease_expires_at, draft_json, draft_sha256, failure_code, completed_at,
  created_at, updated_at
FROM editorial_ai_drafts_0073;

DROP TABLE editorial_ai_drafts_0073;
